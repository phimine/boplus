...
import com.boplus.gen.domain.ColumnConfig;
...
public class GenUtil {
    ...
    public static String download(List<ColumnConfig> columns, GenConfig genConfig) throws IOException {
        String tempPath =System.getProperty("java.io.tmpdir") + "boplus-gen-temp" + File.separator + genConfig.getTableName() + File.separator;
        Map<String,Object> genMap = getGenMap(columns, genConfig);
        TemplateEngine engine = TemplateUtil.createEngine(new TemplateConfig("template", TemplateConfig.ResourceMode.CLASSPATH));
        // 生成后端代码
        List<String> templates = getAdminTemplateNames();
        for (String templateName : templates) {
            Template template = engine.getTemplate("generator/admin/"+templateName+".ftl");
            String filePath = getAdminFilePath(templateName,genConfig,genMap.get("className").toString(),tempPath + "boplus" + File.separator);
        ...
        for (String templateName : templates) {
            Template template = engine.getTemplate("generator/front/"+templateName+".ftl");
            String path = tempPath + "boplus-web"  + File.separator;
